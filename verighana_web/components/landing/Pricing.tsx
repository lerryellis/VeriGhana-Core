...
export function Pricing() {
  ...
  function getPrice(key: string, isAnnual: boolean): string {
    if (key === 'free') return `${currency.symbol}0`
    const base = BASE_USD[key as keyof typeof BASE_USD]
    return fmt(isAnnual ? base.annual : base.monthly, currency)
  }
  ...
  return (
    <section id="pricing" className="py-20 px-[5%] bg-white text-center">
      ...
      {currency.code !== 'USD' && (
        <p className="text-xs text-slate-400 mt-6 font-mono-vg">
          Prices shown in {currency.code} · Charged in GHS via Paystack
        </p>
      )}
    </section>
  )
